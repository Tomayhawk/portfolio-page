    ...
    <div className="max-w-4xl mx-auto py-8">
      ...
      {total === 0 ? (
          ...
          <div className="grid gap-4 animate-fadeIn">
                ...
                <Link key={p.title} to={p.path} className="group relative block bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:border-blue-400/50 dark:hover:border-blue-500/30 hover:-translate-y-0.5">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center gap-2 text-xs font-bold text-zinc-400 dark:text-zinc-500 mb-2 uppercase tracking-wider">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm3.293 1.293a1 1 0 011.414 0L9 9.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-2-2a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                            Page
                        </div>
                        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{p.title}</h2>
            ...
            {blogResults.map(b => (
                <Link key={b.id} to={`/blog/${b.title}`} className="group bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:border-blue-400/50 dark:hover:border-blue-500/30 hover:-translate-y-0.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-zinc-400 dark:text-zinc-500 mb-2 uppercase tracking-wider">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M5.5 13a3.5 3.5 0 01-.369-6.98 4 4 0 117.753-1.3A4.5 4.5 0 1113.5 13H11V9.413l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L9 9.414V13H5.5z" /></svg>
                        Blog
                    </div>
                    ...
                    <p className="text-zinc-600 dark:text-zinc-400">{b.excerpt}</p>
                </Link>
            ))}

            {projectResults.map(p => (
                <Link key={p.id} to={`/projects/${p.title}`} className="group bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:border-blue-400/50 dark:hover:border-blue-500/30 hover:-translate-y-0.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-zinc-400 dark:text-zinc-500 mb-2 uppercase tracking-wider">
            ...
            {photoResults.map(ph => (
                <Link key={ph.id} to="/photography" className="group bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:border-blue-400/50 dark:hover:border-blue-500/30 hover:-translate-y-0.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-zinc-400 dark:text-zinc-500 mb-2 uppercase tracking-wider">
                    ...
                    <p className="text-zinc-600 dark:text-zinc-400">{ph.description}</p>
                </Link>
            ))}
          </div>
      )}
