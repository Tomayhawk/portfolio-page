          ...
          <div className="w-full text-sm text-zinc-800 dark:text-zinc-200 pb-20">
              ...
              <div className="flex flex-col">
                  {sortedProjects.map(p => (
                      <Link to={`/projects/${p.title}`} key={p.id} className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 items-center transition-colors group">
                          ...
                          <div className="col-span-2 flex items-center gap-2 flex-wrap">{p.tags.map(t => <span key={t} className={`inline-block px-2 py-0.5 text-[10px] font-bold rounded ${getTagColor(t)}`}>{t}</span>)}</div>
                          <div className="col-span-2 text-zinc-500 dark:text-zinc-400 text-xs">{new Date(p.modified).toLocaleDateString()}</div>
